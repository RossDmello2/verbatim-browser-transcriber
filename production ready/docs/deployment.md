...
GitHub Pages can serve static files, but this repository currently keeps the maintained app under `production ready/`. If using Pages, either publish that folder through a Pages workflow or flatten the repository deliberately. Do not assume the older root-level files are the maintained app.
